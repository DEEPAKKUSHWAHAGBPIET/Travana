...
<?php $this->load->view('include/website_header'); ?>
	...
	<div class="ibanner_w3 pt-sm-5 pt-3">
		<h4 class="head_agileinfo text-center text-capitalize text-center pt-5">
            ...
            <?php $get_cat_name = get_value('tbl_sub_category','sub_cat_id',$product_data[0]->sub_cat_id,'sub_cat_name');
 ...
 foreach ($get_cat_name_sep as $key => $value2) { ?>
	...
	<nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page"><?php echo $sub_category_data[0]->sub_cat_name; ?></li>
        <?php } ?>
        </ol>
    </nav>
	<!-- //breadcrumbs -->
						...
						<h3 class="shopf-sear-headits-sear-head">Categories</h3>
							<ul>
								<?php foreach ($category as $key => $value) { 
									$datavaluestwo = array('cat_id' => $value->cat_id);
                  $get_sc_count = get_count('tbl_products',$datavaluestwo,'product_id');
									?>
									<li class="own_cat"><a href="<?php echo base_url()."cat_v/".$value->cat_id; ?>"><?php echo $value->category_name." (".$get_sc_count.")"; ?></a></li>
								<?php } ?>
